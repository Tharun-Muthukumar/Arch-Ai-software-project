...
import { metricDirectionLabel } from '../../lib/architectureMetrics'
  ...
  return (
    <div className="panel overflow-x-auto">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-xs" style={{ color: 'var(--text-muted)' }}>Direction is explicit per metric · deterministic model outputs, not objective measurements</span>
      </div>
      ...
      <table className="mt-4 min-w-full text-sm">
        <thead>
          <tr className="border-b" style={{ borderColor: 'var(--card-border)' }}>
            ...
            <th className="py-2 text-left font-medium" style={{ color: 'var(--text-muted)' }}>Direction</th>
        ...
        <tbody>
          {metrics.map((metric) => (
            <tr key={metric.metric} className="border-b" style={{ borderColor: 'var(--card-border)' }}>
              ...
              <td className="py-2 pr-4 text-xs" style={{ color: 'var(--text-muted)' }}>{metricDirectionLabel(metric)}</td>
              {comparison.scorecards.map((scorecard) => {
