            ...
            return (
              <div key={entry.id} className={entry.role === 'user' ? 'chat-row-user' : 'chat-row-assistant'}>
                <div className="chat-bubble">
                  {/* Provenance the backend already computes. A deterministic
                      answer read straight from the project model is a
                      different kind of claim from one the model wrote, and the
                      reader is entitled to see which they got. */}
                  {entry.role === 'assistant' && entry.response ? (
                    <div className="assistant-meta">
                      {entry.response.category ? (
                        <span className={cn('assistant-category', `is-${entry.response.category.toLowerCase()}`)}>
                          {entry.response.category}
                        </span>
                      ) : null}
                      {entry.response.resolved_by ? (
                        <span
                          className={cn(
                            'assistant-resolver',
                            entry.response.resolved_by === 'deterministic' && 'is-deterministic',
                          )}
                          title={
                            entry.response.resolved_by === 'deterministic'
                              ? 'Read directly from the recorded project — no model call'
                              : entry.response.resolved_by === 'fallback'
                                ? 'Retrieved from the recorded project after the model did not answer'
                                : 'Generated by the local model'
                          }
                        >
                          {entry.response.resolved_by}
                          {typeof entry.response.elapsed_ms === 'number'
                            ? ` · ${Math.round(entry.response.elapsed_ms)} ms`
                            : ''}
                        </span>
                      ) : null}
                      {entry.response.confidence ? (
                        <span className="assistant-resolver" title="How far the recorded project supports this answer">
                  ...
                  {entry.response?.evidence_ids?.length ? (
                    <div className="assistant-evidence">
                      <span className="font-mono text-[9.5px] uppercase tracking-wider" style={{ color: 'var(--text-dim)' }}>
                        Evidence
                      </span>
                      {entry.response.evidence_ids.slice(0, 12).map((id) => (
                        <span key={id} className="assistant-evidence-id">{id}</span>
                      ))}
                    </div>
                  ) : null}
